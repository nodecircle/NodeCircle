...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT50000IFMN = 3,
    ONLY_NONDENOMINATED_NOT50000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NCX at the same time
    ONLY_50000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zNCX send
enum ZerocoinSpendStatus {
    ZNCX_SPEND_OKAY = 0,                            // No error
    ZNCX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNCX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNCX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNCX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNCX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNCX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNCX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNCX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNCX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNCX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNCX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNCX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNCX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNCX_SPENT_USED_ZNCX = 14,                      // Coin has already been spend
    ZNCX_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
