...
#if defined(NDEBUG)
#error "NodeCircle cannot be compiled without assertions."
#endif
        ...
        if(tx.IsZerocoinSpend()){
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zNCX spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight > 0 && nHeight <= 90000) {
        nSubsidy = 100 * COIN;
    } else if (nHeight > 90000 && nHeight <= 180000) {
        nSubsidy = 120 * COIN;
    } else if (nHeight > 180000 && nHeight <= 270000) {
        nSubsidy = 150 * COIN;
    } else if (nHeight > 270000 && nHeight <= 360000) {
        nSubsidy = 120 * COIN;
    } else if (nHeight > 360000 && nHeight <= 500000) {
        nSubsidy = 100 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from NodeCircle
...
{
    RenameThread("nodecircle-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZNCXMinted()
{
...
void RecalculateZNCXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zNCX supply
...
bool RecalculateNCXSupply(int nHeightStart)
{
...
bool UpdateZNCXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert NCX to zNCX
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // NodeCircle: We use certain sporks during IBD, so check to see if they are
