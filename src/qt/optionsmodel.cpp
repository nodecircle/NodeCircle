...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NCX);
    ...
    if (!settings.contains("nAnonymizeNodeCircleAmount"))
        settings.setValue("nAnonymizeNodeCircleAmount", 1000);

    nAnonymizeNodeCircleAmount = settings.value("nAnonymizeNodeCircleAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNodeCircleAmount"))
        SoftSetArg("-anonymizenodecircleamount", settings.value("nAnonymizeNodeCircleAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nodecircle.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNodeCircleAmount:
            return QVariant(nAnonymizeNodeCircleAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNodeCircleAmount:
            nAnonymizeNodeCircleAmount = value.toInt();
            settings.setValue("nAnonymizeNodeCircleAmount", nAnonymizeNodeCircleAmount);
            emit anonymizeNodeCircleAmountChanged(nAnonymizeNodeCircleAmount);
            break;
        case CoinControlFeatures:
